[size=16pt][b]Attachment Image Resizing[/b][/size]
[hr]

[color=blue][b][size=12pt][u]License[/u][/size][/b][/color]
o This ElkArte Addon is subject to the terms of the Mozilla Public License version 1.1 (the "License"). You can obtain a copy of the License at http://mozilla.org/MPL/1.1/

[color=blue][b][size=12pt][u]Introduction[/u][/size][/b][/color]
Automatically resizes attached images that are larger than a defined width X height bounding box or any that are larger than the allowed maximum size KB.

Will resize attachment images (.jpg, .png, .gif, .bmp or .webp) to fit within the bounds specified.  The image format will be maintained unless it is unable to fit the resized image within the max allowed file size specified.  In this case, if the optional change format is enabled, the system will convert the image to JPEG for better compression.

Features:
o The ability to set a max width and height for attached images, images are resized proportionately
o The ability to allow converting image formats only if its required to maintain the file within the specified max file size